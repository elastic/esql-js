...
`packages/highlightjs-esql/` (`@elastic/highlightjs-esql`) provides Highlight.js language syntax definitions for ES|QL, pulling keyword/command/function lists from `@elastic/esql-definitions`. Besides the regular `lib/` cjs/esm builds, `yarn build` also emits two self-contained CDN artifacts: `lib/esql.min.js` (self-registers on the global `hljs`) and `lib/esql.es.min.js`.

`packages/textmate-esql/` (`@elastic/textmate-esql`) provides the TextMate grammar for ES|QL (VS Code, Sublime Text, shiki, etc.). The grammar is defined in TypeScript (`src/index.ts`), pulling keyword/function lists from `@elastic/esql-definitions`; `syntaxes/esql.tmLanguage.json` and `syntaxes/esql.tmLanguage` (XML plist) are build artifacts regenerated by `yarn build` — do not edit them by hand.
...
`packages/esql-traversal/` (`@elastic/esql-traversal`) provides ES|QL AST traversal utilities.

`packages/esql-parser/` (`@elastic/esql-parser`) provides the ES|QL query parser.

`packages/esql-ast/` (`@elastic/esql-ast`) provides the ES|QL AST builder.

Unlike the rest of the repo, `@elastic/monaco-esql`, `@elastic/prismjs-esql`, `@elastic/textmate-esql`, and `@elastic/highlightjs-esql` are MIT-licensed (not Elastic-2.0): their `src/` files carry an MIT header instead of the Elastic one, scoped via overrides in `eslint.config.mjs`. Similarly, `@elastic/elasticsearch-query-builder` and `@elastic/elasticsearch-esql-dsl` (migrated from the `elasticsearch-dsl-js` repo) are Apache-2.0-licensed: their `src/` files carry an Apache SPDX header, scoped the same way.
